...
const RecipeCard = ({ recipe, isMyRecipe = false }) => {
  ...
  useEffect(() => {
    if (recipe._id && !isMyRecipe) {
      checkIfRecipeSaved();
    }
  }, [recipe._id, isMyRecipe]);

  const checkIfRecipeSaved = async () => {
    try {
      const result = await userService.checkRecipeSaved(recipe._id);
  ...
  const handleToggleFavorite = async (e) => {
    ...
    if (savingRecipe || isMyRecipe) return;
    
    try {
      setSavingRecipe(true);
      const result = await userService.toggleSavedRecipe({ recipeId: recipe._id });
      setIsRecipeSaved(result.isSaved);
      ...
      <div className="relative overflow-hidden">
        ...
        {!isMyRecipe && (
          <button 
            ...
            className={`absolute top-2 right-2 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 ${
              isRecipeSaved 
          ...
          >
            <svg className={`w-4 h-4 ${isRecipeSaved ? 'text-white fill-current' : 'text-red-500'}`} fill={isRecipeSaved ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-4">
        ...
        <div className="flex space-x-2 mt-3">
          ...
          <button 
            ...
            className="p-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:border-chef-orange dark:hover:border-chef-orange-light hover:text-chef-orange dark:hover:text-chef-orange-light transition-colors text-gray-600 dark:text-gray-400"
            title="Share recipe"
          >
